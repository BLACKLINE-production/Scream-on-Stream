...
fn scan_file(path: &Path, violations: &mut Vec<String>) {
    // Normalize to forward slashes: the exemption lists below are written
    // with `/` separators, but `path.to_string_lossy()` returns native
    // separators (`\` on Windows), so `ends_with`/`contains` against them
    // silently fail to match on Windows and exempt files get flagged.
