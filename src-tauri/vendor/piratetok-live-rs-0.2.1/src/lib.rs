...
impl TikTokLiveBuilder {
    ...
    /// Provide a pre-fetched `ttwid` cookie value, bypassing the automatic
    /// anonymous GET to tiktok.com that normally acquires it.
    ///
    /// Useful when TikTok's anti-bot WAF blocks the automatic fetch from
    /// this client (returns a challenge page with no `Set-Cookie` instead
    /// of the normal response). Grab a fresh value from a real browser's
    /// cookie jar — DevTools → Application → Cookies → `tiktok.com` →
    /// `ttwid` — and pass it here.
    ///
    /// Note: `ttwid` values are time-limited by TikTok and this override
    /// is NOT auto-renewed, so a long-lived connection may still need a
    /// fresh value from time to time.
    pub fn ttwid(mut self, ttwid: impl Into<String>) -> Self {
        self.config.ttwid_override = Some(ttwid.into());
        self
    }
    ...
    pub async fn connect(self) -> Result<TikTokLiveStream, TikTokLiveError> {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let proxy_ref = config.proxy.as_deref();
                let ttwid = if let Some(manual) = &config.ttwid_override {
                    manual.clone()
                } else {
